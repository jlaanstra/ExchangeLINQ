...
{
	public class TagsFilteredByNameContains : ProcessorState<Tag>
	{
		internal TagsFilteredByNameContains(ExchangeUrl url, FilterNameContains name)
		{
			this.Url = url;
			this.Url.QueryUrl = UrlConstants.TagsUrl;
			this.Url.AddQueryOption("inname", string.Join(";", name.Value));
		}
